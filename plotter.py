...
def tricontourf(a, mesh, name, cmap, dpi):
    ...
    if zz[0] != zz.all():
        CS3 = plt.tricontour(xx, yy, triangles, zz, N=10, origin='lower',
                             colors='k')
        plt.clabel(CS3, fontsize=8, colors='k', fmt='%1.1f')

    #plt.xticks(np.arange(min(xx), max(xx)+1, 10.0))
    #plt.yticks(np.arange(min(yy), max(yy)+1, 10.0))
...
def save_pibic(filename):
    plt.savefig('C:/Users/Nasser/OneDrive/UNB/2015_1/pibic/pibic_relatorio'
                '/fig/'+filename+'.eps', bbox_inches='tight',
